...
def load_image_names(filename):
    ...
    for line in doc.split('\n'):  # 按‘\n’将文本文件拆分成列表
        ...
        identifier = line.split('.')[0]  # 去除.jpg后缀
        dataset.append(identifier)
    return set(dataset) # set函数删除元素的重复项
...
def load_clean_captions(filename, dataset):
    ...
    # load document
    doc = load_doc(filename)  # 加载descriptions.txt文件
    descriptions = dict()
    for line in doc.split('\n'):  # 将descriptions.txt的文本数据按每一行划分成一个list
        # split line by white space
        tokens = line.split()  # 将每一行数据按空格进行划分
        # split id from description
        image_id, image_desc = tokens[0], tokens[1:]
        # skip images not in the set
        if image_id in dataset: # 当前图像在训练图像里
            # create list
            if image_id not in descriptions: # 当前图像不是descriptions里面的key
                descriptions[image_id] = list() # 创建key
            # wrap description in tokens
            desc = 'startseq ' + ' '.join(image_desc) + ' endseq' # 添加＇startseq'和＇endseq'
            # store
